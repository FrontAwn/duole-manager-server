...
class NikeController extends Controller {
  ...
  async getChangeList() {
    const ls = await this.service.api.finds(this.ctx.query);
    const count = await this.service.api.count(this.ctx.query);
    this.ctx.body = {
      count,
      ls,
    };
  ...
  async getSkuInfo() {
    const { sku } = this.ctx.query;
    let allsizes = {};
  		let sizes = {};
    let info = await this.service.nike.getInfoBySku(sku);
    const stock = await this.service.nike.getRealTimeStockBySku(sku);
  ...
  async execGenerateStockCommand() {
    const filePath = '/node/duole-erp-cli';
    const fileName = 'nikeToXls.js';
    const res = await exec(`cd ${filePath} && node ${fileName}`, {
  ...
  async downloadStockExcelFile() {
    const stockFile = '/node/duole-erp-cli/excel/output/库存.xlsx';
    const excelBuffer = await readFile(stockFile);
    const currentDayDate = moment().format('YYYYMMDD');
    const excelName = `Nike库存${currentDayDate}.xlsx`;
    const downloadFileName = urlencode(excelName, 'UTF-8');
    this.ctx.set({
